...
| notification.tmux-wake | direct message/resource release/deadline use one atomic `tmux send-keys` subcommand containing the short id and final `Enter`; failure stops permanently at three | no registration, absent, unknown, working, expired, mismatched subject, fourth attempt, body injection, periodic continuation, or second tmux subcommand appears |
