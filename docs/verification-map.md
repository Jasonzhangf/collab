...
| notification.tmux-wake | direct message/resource release/deadline require a subject and use one atomic `tmux send-keys` subcommand containing id, abbreviated subject, safe original-body preview, and final `C-m`; failure stops permanently at three | missing subject, no registration, absent, unknown, working, expired, mismatched subject, fourth attempt, terminal control injection, periodic continuation, or second tmux subcommand appears |
