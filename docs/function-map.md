...
| notification.subscription | `server::state` + `server` | `handle_notification_subscribe`, `matching_subscription`, `handle_notification_unsubscribe` | Agent-owned exact event/subject/TTL; direct-message is a finite reusable lease; resource/deadline/async-result remain one-shot; replayable control truth |
| notification.tmux-wake | `server` + `timers` + `knock` | `abbreviated_subject`, `notification_text`, `attempt_notification_with`, `queue_system_knock`, `wake_args` | registered-only; required subject plus safe original-body preview; one tmux command queue performs unique-buffer bracketed paste then final C-m; direct messages serialize through cooldown; absent/unknown/working reject; hard max three per message |
