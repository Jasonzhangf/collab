...
| notification.tmux-wake | `timers` + `knock` | `attempt_notification_with`, `queue_system_knock`, `wake_args` | registered-only; short id and final Enter in one `send-keys` subcommand; absent/unknown/working reject; hard max three; success consumes |
