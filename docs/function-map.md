...
| notification.tmux-wake | `server` + `timers` + `knock` | `abbreviated_subject`, `notification_text`, `attempt_notification_with`, `queue_system_knock`, `paste_args`, `paste_submit_args`, `literal_args`, `submit_args`, `steer_followup` | registered-only; required subject plus safe original-body preview and execute-not-ACK/wait action suffix; Cursor: two tmux processes, literal payload then C-m after 250ms; working Cursor adds one later empty C-m to steer; Codex: one tmux queue, unique-buffer bracketed paste then C-m; direct messages serialize through cooldown; absent/unknown reject; working Agents receive; hard max three per message |
