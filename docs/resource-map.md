...
| tmux-wake | `server::knock` + `Event::WakeAttempted` | journaled attempt ledger + transient tmux short-id input | only active subscription; absent/unknown/working fail closed; lifetime max three; one atomic `send-keys` subcommand with final `Enter` | body/prompt injection, a second tmux subcommand, automatic continuation, shell input, normal progress report |
